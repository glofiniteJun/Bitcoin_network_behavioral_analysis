        ...
        //
        {
            ...
            for (CNode* pnode : vNodesCopy)
            {
                if (pnode->fDisconnect)
                {
                    ...
                    LogPrint(BCLog::CONFUNDO, "ThreadSocketHandler, %s, is being disconnected , number of incoming: %d , number of outgoing: %d \n", pnode->addr.ToString(), (int)CConnman::GetNodeCountIN_OUT(CConnman::CONNECTIONS_IN), (int)CConnman::GetNodeCountIN_OUT(CConnman::CONNECTIONS_OUT));
                }
            }
        }
        {
    ...
    while (!interruptNet)
    {
        ...
        if (addrConnect.IsValid()) {
            ...
            if (fFeeler) {
                ...
                LogPrint(BCLog::CONFUNDO, "ThreadOpenConnections, Making feeler connection to %s\n", addrConnect.ToString());
            }
...
size_t CConnman::GetNodeCountIN_OUT(NumConnections flags) //final change
{
    LOCK(cs_vNodes);
    int nNum = 0;
    if (flags == CConnman::CONNECTIONS_IN){
        for (const auto& pnode : vNodes) {
            if (pnode->fInbound == true) {
                nNum++;
            }
        } 
    }else if (flags == CConnman::CONNECTIONS_OUT){
        for (const auto& pnode : vNodes) {
            if (pnode->fInbound == false) {
                nNum++;
            }
        }
    }
