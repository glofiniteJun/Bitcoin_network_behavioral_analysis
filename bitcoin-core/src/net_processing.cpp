...
static void PushNodeVersion(CNode *pnode, CConnman* connman, int64_t nTime) //check list
{
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (!pfrom->fInbound)
        {
            ...
            if (pfrom->fOneShot || pfrom->nVersion >= CADDR_TIME_VERSION || connman->GetAddressCount() < 1000)
            {
                ...
                LogPrint(BCLog::CONFUNDO, "ProcessMessage, GETADDR, Sending GETADDR to %s, probably will get ADDR back \n", pfrom->addr.ToString());
                pfrom->fGetAddr = true;
            }
        ...
        if (pfrom->fFeeler) {
            ...
            LogPrint(BCLog::CONFUNDO, "ProcessMessage, VERSION, Disconnecting %s from feeler connection\n ", pfrom->addr.ToString());
            pfrom->fDisconnect = true;
        }
        return true;
    }
    ...
    if (strCommand == NetMsgType::VERACK)
    {
        ...
        if (!pfrom->fInbound) {
          LogPrint(BCLog::CONFUNDO, "ProcessMessage, VERACK, New outgoing connection: peeraddr=%s , number of incoming: %d , number of outgoing: %d \n", pfrom->addr.ToString(), (int)connman->GetNodeCountIN_OUT(CConnman::CONNECTIONS_IN), (int)connman->GetNodeCountIN_OUT(CConnman::CONNECTIONS_OUT)); //final change
        }
        else{
          LogPrint(BCLog::CONFUNDO, "ProcessMessage, VERACK, New incoming connection: peeraddr=%s , number of incoming: %d , number of outgoing: %d \n", pfrom->addr.ToString(), (int)connman->GetNodeCountIN_OUT(CConnman::CONNECTIONS_IN), (int)connman->GetNodeCountIN_OUT(CConnman::CONNECTIONS_OUT)); //final change
        }
    }
    ...
    else if (strCommand == NetMsgType::ADDR)
    {
        ...
        {
            LOCK(cs_main);
             (pfrom->GetId(), 20, strprintf("message addr size() = %u", vAddr.size()));
            return false;
        }
        ...
        LogPrint(BCLog::CONFUNDO, "ProcessMessage, ADDR, Receiving ADDR from %s: include %d IPs, inserting %d IPs to new \n", pfrom->addr.ToString(), vAddr.size(), vAddrOk.size()); 
